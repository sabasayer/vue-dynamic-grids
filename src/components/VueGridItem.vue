  ...
  <div :class="classes" :style="styles">
    ...
    >
      <div
        ...
        ref="component"
      ></div>
      <slot :cols="cols" :w="w" :h="h" v-else></slot>
    </DraggableCore>
    <Resizable
      v-if="isResizable"
      :w="calcWidth()"
...
export default class GridItem extends Vue {
  ...
  public heightObserver(mutationsList, observer) {
    ...
    for (const mutation of mutationsList) {
      if (
        ["attributes", "characterData", "childList", "subtree"].indexOf(
          mutation.type
        ) > -1
      ) {
        let newHeight = (this.$slots.default[0] as any).elm.clientHeight;
        ...
        if (this.componentHeight !== newHeight) {
          ...
          this.$nextTick(() => {
            ...
            this.resizing = null;
            this.eventBus.$emit("onResizeStop", this, this.i, w, h, {
              e: null,
              node: null
            });
          });
  ...
  public onDragHandler(handlerName) {
    return (e, { node, deltaX, deltaY }) => {
      ...
      this.eventBus.$emit(handlerName, this, this.i, x, y, {
        e,
        node,
        newPosition
      });
